...
def write_to_csv(file_name, funghi_entry_list, use_intervals):
    ...
    # Write header line including family and name
    if not use_intervals:
        file.write("family;name;" + data_cat.PRIMARY_DATASET_HEADER + "\n")
    else:
        file.write("family;name;" + data_cat.DATASET_HEADER_MIN_MAX + "\n")

    # Write each entry's data to the file
    for funghi_entry in funghi_entry_list:
        # Include family and name in the output line
        funghi_str = (
            f"{funghi_entry.family};{funghi_entry.name};{funghi_entry.is_edible}"
        )
...
def generate_funghi_entry_list(funghi_type_dict, number, use_intervals):
    ...
    for funghi_key in funghi_type_dict:
        funghi_type_categories_list = list(
            funghi_type_dict[funghi_key].categories_dict.values()
        )
        funghi_class_list = [
            funghi_type_dict[funghi_key].family,
        ...
        for entry_count in range(0, number):
            ...
            for category_count in range(0, len(funghi_type_categories_list)):
                ...
                if category_count not in metrical_attributes_columnindex_dict.keys():
                    funghi_entry_attributes_list.append(
                        random.choice(funghi_type_categories_list[category_count])
                    )
                # metrical values
                else:
                    ...
                    if not use_intervals:
                        funghi_entry_attributes_list.append(
                            round(
                                normal_values[
                                    metrical_attributes_columnindex_dict[category_count]
                                ][entry_count],
                                2,
                            )
                        )
                    # put interval borders in separate categories
                    else:
                        funghi_entry_attributes_list.append(
                            funghi_type_categories_list[category_count][0]
                        )
                        funghi_entry_attributes_list.append(
                            funghi_type_categories_list[category_count][1]
                        )
...
def generate_funghi_entry_dummies_list(number):
    ...
    funghi_entry_list = []
    funghi_entry_attributes_list_e = [
        "Family",
        "Dummy Shroom",
        "e",
        17.2,
    ...
    funghi_entry_attributes_list_p = [
        "Family",
        "Deadly Dummy Shroom",
        "p",
        17.2,
...
if __name__ == "__main__":
    ...
    edited_funghi_type_dict = primary_data_gen.get_funghi_type_dict_from_csv(
        data_cat.FILE_PATH_PRIMARY_EDITED, 0, 173
    )

    funghi_entry_list = generate_funghi_entry_list(edited_funghi_type_dict, 353, False)
    funghi_entry_list_with_intervals = generate_funghi_entry_list(
        edited_funghi_type_dict, 353, True
    )
    ...
    #     data_cat.FILE_PATH_SECONDARY_GENERATED, sep=";", header=0, low_memory=False
    # )
