...
# from altair import pipe

import src.util_func as util_func
...
def get_balance_chart(data, **kwargs):
    ...
    if kwargs["count"]:
        size = len(data)
        val_counts = data["name"].value_counts()
        if kwargs["reindex"]:
    ...
    chart = (
        alt.Chart(data, title=kwargs["title"])
        .mark_bar(size=150)
        .encode(
...
def get_correlation_dataframe(data, **kwargs):
    ...
    data_corr = pd.DataFrame(
        columns=["variable1", "variable2", "correlation", "correlation_rounded"]
    )

    for variable1 in data:
        for variable2 in data:
            if variable1 <= variable2:  # Only compute for half the matrix
                # nominal-nominal -> Theils U
                if isinstance(data[variable1].iloc[0], str) and isinstance(
                    data[variable2].iloc[0], str
                ):
                    corr = nominal.theils_u(
                        data[variable1], data[variable2], nan_replace_value="f"
                    )

                # metrical-metrical -> Pearsons R
                elif util_func.is_number(
                    data[variable1].iloc[0]
                ) and util_func.is_number(data[variable2].iloc[0]):
                    corr = scipy.stats.stats.pearsonr(data[variable1], data[variable2])[
                        0
                    ]
                    corr = (corr + 1) / 2  # Scale from [-1,1] to [0,1]

                # mixed types -> correlation ratio
                elif isinstance(data[variable1].iloc[0], str) and util_func.is_number(
                    data[variable2].iloc[0]
                ):
                    corr = nominal.correlation_ratio(
                        data[variable1], data[variable2], nan_replace_value="f"
                    )
                elif isinstance(data[variable2].iloc[0], str) and util_func.is_number(
                    data[variable1].iloc[0]
                ):
                    corr = nominal.correlation_ratio(
                        data[variable2], data[variable1], nan_replace_value="f"
                    )
                ...
                data_corr = pd.concat([data_corr, pd.DataFrame(new_row, index=[0])])
...
def get_score_threshold_dataframe(X_train, X_test, y_train, y_test, mode, score):
    ...
    for threshold in data.thresholds:
        y_prob, y_pred = mushroom_class_fix.get_y_prob_pred(
            X_test, model, threshold=threshold
        )
        scores.append(
            mushroom_class_fix.get_evaluation_scores_dict(y_test, y_pred, print=False)[
                score
            ]
        )
    data.scores = scores
...
def get_score_threshold_chart(X_train, X_test, y_train, y_test, mode, score):
    ...
    chart = (
        ...
        .mark_line()
        .encode(alt.X("thresholds:Q"), alt.Y("scores:Q"), color=alt.value("black"))
    )
    return chart
...
def get_roc_dataframe(X_train, X_test, y_train, y_test, mode):
    ...
    false_positive_rate, true_positive_rate, thresholds = mushroom_class_fix.get_roc(
        y_test, y_prob
    )
    for i in range(0, len(false_positive_rate)):
        new_row = {
            "true positive rate": true_positive_rate[i],
...
def get_roc_chart(X_train, X_test, y_train, y_test, mode, **kwargs):
    ...
    if "title" not in kwargs:
        kwargs["title"] = (
            "ROC curve for "
            + mushroom_class_fix.mode_dict[mode]
            + ", AUC = %0.2f"
            % auc(data_roc["false positive rate"], data_roc["true positive rate"])
        )
    line_chart = (
        alt.Chart(data_roc, title=kwargs["title"])
        .mark_line()
        .encode(
...
if __name__ == "__main__":
    ...
    data_original = pd.read_csv(
        data_cat.FILE_PATH_1987_NO_MISS, sep=";", header=0, dtype=object
    )
    ...
    # threshold charts
    threshold_chart = get_score_threshold_chart(
        X_train, X_test, y_train, y_test, "nb", "F2"
    )
